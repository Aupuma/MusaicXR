Shader "Brush/Line3S"
{
    Properties
    {
        ...
        _Speed ("Speed of waves", Float) = 10.0
        _BaseColour ("Base Colour", Vector) = (0.3, 0.1, 0.7, 1.0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float wave_fuzzy(float2 uv, float time, float amp, float thickness, float offset, float freq, float velocity) {
                float y = amp * 0.5 * cos(uv.x * freq + time * _Speed * velocity) + 0.5 + offset;
                // if (uv.y < y + thickness && uv.y > y - thickness) {
                //     return 1.0;
                // }
                // return 0.0;
                // float mid = 0.5 - 0.5 * thickness;
                return 1.0 - plateau(abs(uv.y - y), (0.5 * thickness) - 0.05, (0.5 * thickness) + 0.05);
            }
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float w1 = wave_fuzzy(i.uv, _Time.y, 0.6, 0.2, 0.0, 500.0, 1.0);
                ...
                float fade = plateau2d(i.uv.y, 0.0, 0.5);
                float middle_line = plateau2d(i.uv.y, 0.4, 0.45);

                float a = fade * 0.7 + middle_line * 0.9 + w1 * 1.0;
